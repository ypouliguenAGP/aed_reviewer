...
def processAEDConfig(basedir):
    ...
    conn_events = sqlite3.connect(f'{FOLDER_NAME}/tuba/events.db')
    ...
    for row in results:
        ...
        if row['local_alert_type'] == 200:
            pgs[row['pgid']]['alert_thresholds']['total'] = {
                'mode': BANDWIDTH_ALERT_MODES[row['enable_mode']],
    ...
    for row in results:
        ...
        for type_id in notification_config_results:
            types_human.append(NOTIFICATION['type'][type_id])
            ...
            for key in row:
                if key.startswith("syslog"):
                    if key == "syslog_severity":
                        obj[key[7:]] = NOTIFICATION['severity'][row[key]]
                        continue
                    if key == "syslog_facility":
                        obj[key[7:]] = NOTIFICATION['facility'][row[key]]
                        continue
                    obj[key[7:]] = row[key]
