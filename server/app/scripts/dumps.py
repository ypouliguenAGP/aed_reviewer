...
def processPacketDump(pg_id, FOLDER_NAME):

    if not os.path.exists(f"{FOLDER_NAME}/stats/dumps/{pg_id}.log"):
        print(f"File {FOLDER_NAME}/stats/dumps/{pg_id}.log does not exit")
        raise FileNotFoundError(f"File {FOLDER_NAME}/stats/dumps/{pg_id}.log does not exit")

    entries = []        
    with open(f"{FOLDER_NAME}/stats/dumps/{pg_id}.log") as f:
        position = 1
        entry = {'#': position}
        for line in f:
            if line == "\n":
...
def packetElementStats(entries, element_name, unit='bps'):
    ...
    for entry in entries:
        ...
        if 'port' in element_name:
            entry_name = f"{PROTO[int(entry['proto'])]}/{entry[element_name]}"
        else:
    ...
    if len(selected_elements) <= MAX_DUMPS_STATS_ELEMENTS:
        return selected_elements
    # Calculating others count
    total_other = 0
    for selected_element in selected_elements[MAX_DUMPS_STATS_ELEMENTS:len(selected_elements)]:
        total_other += selected_element[1]
    return selected_elements[:MAX_DUMPS_STATS_ELEMENTS] + [('others', total_other)]
